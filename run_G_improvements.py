...
class NetG_Ablation(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.cbf = nn.Sequential(nn.Linear(dim, 128), nn.ELU(), nn.Linear(128, 128), nn.ELU(), nn.Linear(128, 128), nn.ELU(), nn.Linear(128, 128), nn.ELU(), nn.Linear(128, 128), nn.ELU(), nn.Linear(128, 1))
        self.att = nn.Sequential(nn.Linear(dim, 64), nn.ELU(), nn.Linear(64, 64), nn.ELU(), nn.Linear(64, 64), nn.ELU(), nn.Linear(64, 64), nn.ELU(), nn.Linear(64, 1))
    def forward(self, x): return torch.cat([self.cbf(x), self.att(x)], dim=1)

results = []
for t in ['diff', 'log', 'ratio', 'all']:
    xtr, xte, dim = get_data(t)
    res = train_eval(NetG_Ablation(dim).to(device), xtr, xte, f"G3_Feature_{t}")
    results.append(res)
    print(res)
